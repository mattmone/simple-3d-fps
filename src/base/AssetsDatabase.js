...
import '../../web_modules/@babylonjs/loaders.js';
export default class AssetsDatabase {
  ...
  addSound(name, file, options) {
    ...
    fileTask.onSuccess = (task) => {
      this.sounds[name] = new Sound(name, task.data, this.scene, null, options);
  ...
  addMesh(name, file, options = {}, mergeMeshes = false) {
    ...
    fileTask.onSuccess = (task) => {
      console.log(`${name} mesh added`);
      let mesh = null;

      try {
        if (mergeMeshes) {
          mesh = Mesh.MergeMeshes(task.loadedMeshes);
        } else {
